...
object PlayerSize : Module(
    ...
    fun preRenderCallbackScaleHook(entityRenderer: PlayerEntityRenderState, matrix: MatrixStack) {
        if (enabled && entityRenderer.displayName?.string == mc.player?.name?.string && !randoms.containsKey(entityRenderer.displayName?.string)) {
            if (devSizeY < 0) matrix.translate(0f, devSizeY * 2, 0f)
            matrix.scale(devSizeX, devSizeY, devSizeZ)
        }
        if (!randoms.containsKey(entityRenderer.displayName?.string)) return
        if (!devSize && entityRenderer.displayName?.string == mc.player?.name?.string) return
        val random = randoms[entityRenderer.displayName?.string] ?: return
